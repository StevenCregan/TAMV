...
def getBuffer():
    startTime = time.time()
    response = session.get( duetAddress + '/rr_gcode', timeout=timeoutSeconds ).json()
    duration = time.time() - startTime
    reply = session.get( duetAddress + '/rr_reply', timeout=timeoutSeconds )
...
def move():
    startTime = time.time()
    response = session.get( duetAddress + '/rr_gcode?gcode=G91 G1 X0.5 Y0.5 F3000 G90', timeout=timeoutSeconds ).json()
    duration = time.time() - startTime
    reply = session.get( duetAddress + '/rr_reply', timeout=timeoutSeconds )
    buffer_size = int(response['buff'])
    logging.info( 'Query took: %.3f s.' % duration )
...
def homePrinter():
    ...
    response = session.get( duetAddress + '/rr_gcode?gcode=G28 XY', timeout=timeoutSeconds ).json()
    duration = time.time() - homestartTime
    reply = session.get( duetAddress + '/rr_reply', timeout=timeoutSeconds )
    while True:
        startTime = time.time()
        response = session.get( duetAddress + '/rr_status?type=2', timeout = timeoutSeconds ).json()
        reply = session.get( duetAddress + '/rr_reply', timeout=timeoutSeconds )
        duration = time.time() - homestartTime
    ...
    response = session.get( duetAddress + '/rr_gcode?gcode=G1 X0 Y0 F3000', timeout=timeoutSeconds )
    return
...
def getCoords():
    ...
    while True:
        try:
            response = session.get( duetAddress + '/rr_status?type=2', timeout = timeoutSeconds ).json()
            reply = session.get( duetAddress + '/rr_gcode', timeout=timeoutSeconds ).json()
...
if __name__=='__main__':
    ...
    startTime = time.time()
    response = session.get( duetAddress + '/rr_connect?password=reprap', timeout=timeoutSeconds )
    duration = time.time() - startTime
    getBuffer()
    logging.info( 'Query took: %.3f s.' % duration )
    ...
    for i in range(loops):
        ...
        startTime = time.time()
        while True:
            response = session.get( duetAddress + '/rr_status?type=2', timeout = timeoutSeconds )
            reply = session.get( duetAddress + '/rr_reply', timeout=timeoutSeconds )
